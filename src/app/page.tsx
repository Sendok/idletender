...
function getStatusAccent(status: string): string {
  const accents: Record<string, string> = {
    ...
    SHORTLISTED: 'border-l-amber-500',
          ...
          <div className="hidden md:flex items-center gap-1">
            {!isAuthenticated ? (
              ...
              <>
                ...
                <Separator orientation="vertical" className="h-6" />
                <Button variant="ghost" size="sm" onClick={() => navigate('provider-profile')}>
                  <Avatar className="w-6 h-6 mr-1"><AvatarFallback className="text-xs bg-emerald-100 text-emerald-700">{user?.name?.charAt(0)}</AvatarFallback></Avatar>
                  {user?.name}
                </Button>
              ...
              <div className="flex flex-col gap-2 pt-3">
                {!isAuthenticated ? (
                  ...
                  <>
                    ...
                    <Button variant="ghost" className="justify-start" onClick={() => { navigate('provider-profile'); setMobileMenuOpen(false) }}><User className="w-4 h-4 mr-2" /> Profile</Button>
                    <Button variant="ghost" className="justify-start" onClick={handleLogout}><LogOut className="w-4 h-4 mr-2" /> Logout</Button>
                  </>
                )}
              </div>
...
function StatCard({ label, value, icon: Icon, color = 'emerald' }: { label: string; value: number | string; icon: any; color?: string }) {
  const colors: Record<string, { iconBg: string; cardBg: string }> = {
    emerald: { iconBg: 'bg-emerald-100 text-emerald-600', cardBg: 'bg-gradient-to-br from-emerald-50 to-white' },
    blue: { iconBg: 'bg-blue-100 text-blue-600', cardBg: 'bg-gradient-to-br from-blue-50 to-white' },
    amber: { iconBg: 'bg-amber-100 text-amber-600', cardBg: 'bg-gradient-to-br from-amber-50 to-white' },
    purple: { iconBg: 'bg-purple-100 text-purple-600', cardBg: 'bg-gradient-to-br from-purple-50 to-white' },
    red: { iconBg: 'bg-red-100 text-red-600', cardBg: 'bg-gradient-to-br from-red-50 to-white' },
  }
  const c = colors[color] || colors.emerald
  return (
    <Card className={c.cardBg}>
...
function StarRating({ rating, onRate, size = 'w-6 h-6' }: { rating: number; onRate: (r: number) => void; size?: string }) {
    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
          className={`transition-colors ${star <= (hover || rating) ? 'text-amber-400' : 'text-gray-300'} hover:text-amber-400`}
...
function ReviewDialog({ jobId, jobTitle, onSubmitted }: { jobId: string; jobTitle: string; onSubmitted: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await reviewsApi.create({ jobId, rating, comment }, token)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Review: {jobTitle}</DialogTitle>
          <DialogDescription>Rate your experience and share feedback</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <Button className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={handleSubmit} disabled={submitting || rating === 0}>
...
function ReviewsSection({ reviews, avgRating, totalReviews, compact = false }: { reviews: any[]; avgRating?: number; totalReviews?: number; compact?: boolean }) {
  if (reviews.length === 0 && totalReviews === undefined) return null
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          {avgRating !== undefined && avgRating > 0 && (
            <div className="flex items-center gap-1 bg-amber-50 px-3 py-1 rounded-full">
              ...
              <span className="font-semibold text-amber-700">{avgRating.toFixed(1)}</span>
              <span className="text-sm text-amber-600">({totalReviews || 0})</span>
            </div>
          )}
        </div>
      )}
      {compact && avgRating !== undefined && avgRating > 0 && (
        <div className="flex items-center gap-2 mb-3">
          <Star className="w-5 h-5 text-amber-500 fill-amber-500" />
          <span className="text-lg font-bold">{avgRating.toFixed(1)}</span>
          <span className="text-sm text-muted-foreground">({totalReviews || 0} reviews)</span>
        </div>
      )}
      {reviews.length === 0 ? (
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {reviews.map((review: any) => (
            <Card key={review.id} className="hover:shadow-sm transition">
  ...
  const testimonials = [
    {
      ...
      initials: 'SC',
      quote: 'TenderHub changed how I find work. Instead of competing on price, I can showcase my expertise and let the quality of my proposals speak for itself.',
    },
    {
      name: 'Marcus Johnson',
      role: 'Startup Founder',
      initials: 'MJ',
      quote: 'As a job provider, I finally get thoughtful proposals instead of spam bids. The quality of workers on this platform is outstanding.',
    },
    {
      name: 'Aisha Patel',
      role: 'UX Designer',
      initials: 'AP',
      quote: 'I landed my dream project within a week of joining. The proposal-based system means clients actually read my approach before deciding.',
    },
  ]

  const trustedCompanies = ['Acme Corp', 'TechFlow', 'DataPulse', 'CloudSync', 'InnoVate']
  ...
  return (
    <div>
      ...
      <section className="py-10 bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-center text-sm text-muted-foreground mb-6 uppercase tracking-wider font-medium">Trusted by leading companies</p>
          <div className="flex flex-wrap justify-center items-center gap-8 sm:gap-12">
            {trustedCompanies.map((company) => (
              <div key={company} className="flex items-center gap-2 text-gray-400 hover:text-gray-600 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}>
                <Card className="text-center h-full hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { label: 'Active Jobs', value: '500+', icon: Briefcase, color: 'emerald' },
              { label: 'Skilled Workers', value: '2,000+', icon: Users, color: 'blue' },
              { label: 'Proposals Sent', value: '10,000+', icon: FileText, color: 'amber' },
              { label: 'Jobs Completed', value: '3,500+', icon: CheckCircle, color: 'purple' },
            ].map((stat, i) => (
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg">Real stories from workers and job providers</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                <Card className="h-full hover:shadow-lg hover:scale-[1.02] transition-all duration-300">
                  <CardContent className="p-6">
                    ...
                    <div className="flex items-center gap-3">
                      <Avatar className="w-10 h-10">
                        <AvatarFallback className="bg-emerald-100 text-emerald-700 font-semibold">{t.initials}</AvatarFallback>
                      </Avatar>
                      <div>
      ...
      {/* CTA */}
      {!isAuthenticated && (
        <section className="py-20 bg-gradient-to-br from-emerald-600 via-teal-600 to-emerald-800 text-white relative overflow-hidden">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20" />
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl sm:text-4xl font-bold mb-4">Ready to Get Started?</h2>
              <p className="text-lg text-emerald-100 mb-8">Join thousands of workers and job providers on TenderHub</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button size="lg" className="bg-white text-emerald-700 hover:bg-emerald-50 font-semibold text-lg px-8 h-12" onClick={() => navigate('register')}>
                  Join as Worker
                </Button>
                <Button size="lg" variant="outline" className="border-white/40 text-white hover:bg-white/10 font-semibold text-lg px-8 h-12" onClick={() => navigate('register')}>
...
function BrowseJobsPage() {
  ...
  const hasActiveFilters = search || category || status || budgetMin || budgetMax
  ...
  useEffect(() => {
    ...
    jobsApi.list(params).then(res => {
      if (!cancelled) {
        ...
        // Client-side sort for non-DB-supported sorts
        if (sortBy === 'budget_high') {
          filtered = [...filtered].sort((a: any, b: any) => b.budgetMax - a.budgetMax)
        } else if (sortBy === 'budget_low') {
          filtered = [...filtered].sort((a: any, b: any) => a.budgetMin - b.budgetMin)
        } else if (sortBy === 'deadline') {
          filtered = [...filtered].sort((a: any, b: any) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime())
        } else if (sortBy === 'proposals') {
          filtered = [...filtered].sort((a: any, b: any) => (b._count?.proposals || 0) - (a._count?.proposals || 0))
        }
        setJobs(filtered)
  ...
  }, [page, search, category, status, budgetMin, budgetMax, sortBy])
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="space-y-4 mb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-end">
          ...
          {hasActiveFilters && (
            <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700" onClick={clearFilters}>
      ...
      {!loading && (
        <p className="text-sm text-muted-foreground mb-4">
          Showing {jobs.length} of {totalResults} job{totalResults !== 1 ? 's' : ''}
        </p>
      )}
...
function JobDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setLoading(false)
      try {
        const revRes = await reviewsApi.getJobReviews(params.id)
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {isOwner && proposals.length > 0 && (
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {proposals.map((p: any) => (
                <ProposalCard key={p.id} proposal={p} isOwner={isOwner} jobId={job.id} isWorkerView={false} onRefresh={fetchJob} />
              ))}
            </div>
          </div>
        )}
...
function ProposalCard({ proposal, isOwner, jobId, isWorkerView, onRefresh }: { proposal: any; isOwner: boolean; jobId: string; isWorkerView: boolean; onRefresh: () => void }) {
  ...
  const handleWithdraw = async () => {
    ...
    setLoading(true)
    try {
      await proposalsApi.updateStatus(proposal.id, 'WITHDRAWN', token)
  ...
  return (
    <Card className={`hover:shadow-sm transition border-l-4 ${getStatusAccent(proposal.status)}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <span className="text-xs text-muted-foreground ml-auto">{timeAgo(proposal.createdAt)}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {isWorkerView && (proposal.status === 'SUBMITTED' || proposal.status === 'SHORTLISTED') && (
                <Button size="sm" variant="outline" className="text-amber-600 hover:text-amber-700 hover:bg-amber-50" onClick={handleWithdraw} disabled={loading}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {data?.recentProposals?.length > 0 ? (
            <div className="space-y-3">
              {data.recentProposals.map((p: any) => (
                <Card key={p.id} className={`cursor-pointer hover:shadow-md hover:scale-[1.01] transition-all border-l-4 ${getStatusAccent(p.status)}`} onClick={() => navigate('job-detail', { id: p.jobId })}>
                  <CardContent className="p-4">
            ...
            <Card className="text-center py-8">
              <CardContent>
                <FileText className="w-16 h-16 mx-auto mb-3 text-muted-foreground/40" />
                <h3 className="text-lg font-semibold mb-1">No proposals yet</h3>
                <p className="text-muted-foreground mb-3">Start browsing jobs and submit your first proposal</p>
                <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => navigate('browse-jobs')}>Find Jobs</Button>
              </CardContent>
            </Card>
          )}
        </div>
        ...
        <div>
          ...
          {data?.activeJobs?.length > 0 ? (
            ...
            <Card className="text-center py-8">
              <CardContent>
                <Briefcase className="w-16 h-16 mx-auto mb-3 text-muted-foreground/40" />
                <h3 className="text-lg font-semibold mb-1">No active jobs</h3>
                <p className="text-muted-foreground">Keep submitting proposals to get assigned to jobs</p>
              </CardContent>
            </Card>
          )}
        </div>
...
function WorkerProposalsPage() {
  ...
  const handleWithdraw = async (proposalId: string) => {
    if (!token) return
    try {
      await proposalsApi.updateStatus(proposalId, 'WITHDRAWN', token)
      toast.success('Proposal withdrawn')
      proposalsApi.getMyProposals(token, statusFilter || undefined).then(res => setProposals(res.proposals))
    } catch (error: any) {
      toast.error(error.message || 'Failed to withdraw proposal')
    }
  }
      ...
      {loading ? (
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">Start submitting proposals to jobs that match your skills</p>
        ...
        <div className="space-y-4">
          {proposals.map((p: any) => (
            <Card key={p.id} className={`hover:shadow-md transition cursor-pointer border-l-4 ${getStatusAccent(p.status)}`} onClick={() => navigate('job-detail', { id: p.jobId })}>
              <CardContent className="p-4 sm:p-6">
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="active" className="mt-4">
          {loading ? <Skeleton className="h-32" /> : activeJobs.length === 0 ? (
            <Card className="text-center py-12">
              <CardContent>
                <Briefcase className="w-16 h-16 mx-auto mb-4 text-muted-foreground/40" />
                <h3 className="text-lg font-semibold mb-1">No active jobs</h3>
                <p className="text-muted-foreground">Submit proposals to get assigned to jobs</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
...
function WorkerProfilePage() {
  ...
  useEffect(() => {
    if (user?.id) {
      usersApi.getReviews(user.id).then(res => {
        setReviews(res.reviews)
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {reviews.length > 0 && (
        <div className="mt-8">
          <ReviewsSection reviews={reviews} avgRating={avgRating} totalReviews={totalReviews} />
        </div>
      )}
    </div>
  )
...
function ProviderDashboardPage() {
  ...
  const handleCancelJob = async (jobId: string) => {
    if (!token) return
    try {
      await jobsApi.update(jobId, { status: 'CANCELLED' }, token)
      toast.success('Job cancelled')
        ...
        {data?.recentJobs?.length > 0 ? (
          <div className="space-y-4">
            {data.recentJobs.map((job: any) => (
              <Card key={job.id} className={`hover:shadow-md transition border-l-4 ${getStatusAccent(job.status)}`}>
                <CardContent className="p-4 sm:p-6">
                  <div className="flex flex-col sm:flex-row justify-between gap-4">
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-4">Post your first job to start receiving proposals from skilled workers</p>
      ...
      {loading ? <div className="space-y-4">{[1,2,3].map(i => <Skeleton key={i} className="h-32" />)}</div> : jobs.length === 0 ? (
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">Post your first job to start receiving proposals from talented workers</p>
            <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => navigate('provider-create-job')}>Post Your First Job</Button>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
      ...
      {proposals.length === 0 ? (
        <Card className="text-center py-12">
          <CardContent>
            <FileText className="w-16 h-16 mx-auto mb-4 text-muted-foreground/40" />
            <h3 className="text-lg font-semibold mb-2">No proposals yet</h3>
            <p className="text-muted-foreground">Workers will start submitting proposals soon. Make sure your job description is detailed and appealing.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          {proposals.map((p: any) => (
            <ProposalCard key={p.id} proposal={p} isOwner={true} jobId={params.id} isWorkerView={false} onRefresh={fetchData} />
          ))}
        </div>
      )}
    </div>
      ...
      <Card>
        <CardContent className="p-6 space-y-6">
          ...
          <div>
            ...
            <Textarea value={form.bio} onChange={(e) => setForm(f => ({ ...f, bio: e.target.value }))} rows={4} placeholder="Tell us about your company..." />
          </div>
          <div>
            <Label>Location</Label>
            <Input value={form.location} onChange={(e) => setForm(f => ({ ...f, location: e.target.value }))} placeholder="City, Country" />
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {loading ? (
        ...
        <Card className="text-center py-12">
          <CardContent>
            <Bell className="w-16 h-16 mx-auto mb-4 text-muted-foreground/40" />
            <h3 className="text-lg font-semibold">No notifications</h3>
            <p className="text-muted-foreground">You&apos;re all caught up! Notifications about your jobs and proposals will appear here.</p>
          </CardContent>
        </Card>
      ) : (
